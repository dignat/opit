...
class ${className}Controller {
    ...
    def list(Integer max) {
            def s = new ${className}()
        if (s.hasProperty("tenantId")) {
            def client = Client.findByTenantId(session.tenantId) 
            def items = ${className}.findAllByTenantId(session.tenantId) as List
            params.max = Math.min(max ?: 10, 100)
            [${propertyName}List: items, ${propertyName}Total: items.size()] 
        }
        else {
            params.max = Math.min(max ?: 10, 100)
...
//    }
    ...
    def save = {
        params.tenantId = session.tenantId.toLong()     
        def ${propertyName} = new ${className}(params) 
//		${propertyName}.getMetaClass().getMethods().each {
//			if (it.toString().indexOf('get') != -1)
//				println it
//		}
			
     	if (${propertyName}.save(flush: true,failOnError:true)) {
            flash.message = "\${message(code: 'default.created.message', args: [message(code: '${domainClass.propertyName}.label', default: '${className}'), ${propertyName}.id])}"
            redirect(action: "show", id: ${propertyName}.id)
        }
        else {
    ...
    def show(Long id) {      
        def ${propertyName} = ${className}.get(id)
           def s = new ${className}()
        if (s.hasProperty("tenantId")) {
            def tenantId = session.tenantId.toLong()
            if (${propertyName}.tenantId != tenantId ){
