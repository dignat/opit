...
class WeeklyStatsJob {
    static triggers = {
      cron cronExpression: '0 0 */3 * * ?' // every 3hrs
    }
        ...
        // execute job
			...
			dataCenter.each {
				...
				def vm = new WeeklyStats(virtualMachine:vmname,description:os, cpuCount:cpuCount, memory:memory, memoryOverhead:memoryOverhead, state:state, vmWareToolStatus:vmWareToolStatus, ipAddress:ip, 
